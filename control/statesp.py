...
class StateSpace(NonlinearIOSystem, LTI):
    ...
    * `dt` = None: no timebase specified
    ...
    def lft(self, other, nu=-1, ny=-1, **kwargs):
        ...
        ny : int, optional
            Dimension of (plant) measurement output that is connected
            to `other`.  Must not exceed the number of outputs of
            `self` or the number of inputs of `other`.  If not
            specified, defaults to the largest value allowed by the
            shapes of `self` and `other`.
        nu : int, optional
            Dimension of (plant) control input that is generated by
            `other`.  Must not exceed the number of inputs of `self`
